...
func _init() -> void:
	...
	assert(board.project(Vector2.ZERO).distance_to(Vector2(352, 180)) < 0.1)
	assert(board.project(Vector2(864, 672)).distance_to(Vector2(910, 614)) < 0.1)
	...
	for point: Vector2 in [Vector2(351,180),Vector2(911,180),Vector2(352,615),Vector2(911,614),Vector2(500,179),Vector2(500,615)]:
	...
	assert(cell_size == Vector2(62,62) and board.hit_cells.size() == 63)
